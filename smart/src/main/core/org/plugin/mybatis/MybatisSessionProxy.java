...
/**
 * Mybatis - 代理类，用于自动处理SqlSession Created by liuzh on 14-4-9.
 */
			...
			for (Field beanField : beanFields) {
				if (beanField.isAnnotationPresent(MybatisInject.class)) {
					try {
						...
						beanField.set(BeanHelper.getBean(cls), MybatisHelper.getMapper(beanField.getType()));
						flag = true;
					} catch (IllegalArgumentException | IllegalAccessException e) {
						logger.debug("注入Mybatis的Session失败，失败的类：" + cls + "\n失败信息：" + e.getMessage());
					}
				}
			}
			if (!flag) {
