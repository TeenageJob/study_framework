            ...
            for (Map.Entry<Class<?>, Object> beanEntry : beanMap.entrySet()) {
                ...
                if (ArrayUtil.isNotEmpty(beanFields)) {
                    ...
                    for (Field beanField : beanFields) {
                        ...
                        } else if (beanField.isAnnotationPresent(Autowired.class)) {
                            //循环遍历有Bean注解的类
                            for (Map.Entry<Class<?>, Object> bean : BeanHelper.getBeanClass().entrySet()) {
                                // 获取 Bean 类与 Bean 实例
                                Class<?> bClass = bean.getKey();
                                Object bInstance = bean.getValue();
                                // 获取 Bean 类中所有的字段（不包括父类中的方法）
                                Field[] bFields = bClass.getDeclaredFields();
                                try {
                                    for (Field field : bFields) {
                                        if (beanField.getName().equals(field.getName())) {
                                            field.setAccessible(true);
                                            beanField.setAccessible(true); // 将字段设置为 public
                                            Object value = field.get(bInstance);
                                            beanField.set(beanInstance, value); // 设置字段初始值
                                        }
                                    }
                                } catch (Exception e) {
                                    e.printStackTrace();
