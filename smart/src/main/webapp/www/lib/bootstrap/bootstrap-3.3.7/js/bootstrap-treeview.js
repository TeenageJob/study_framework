...
;(function ($, window, document, undefined) {
    ...
    var Tree = function (element, options) {
        ...
        return {
            ...
            //button function
            addChrildrenNode: $.proxy(this.addChrildrenNode, this),
            delThisNode: $.proxy(this.delThisNode, this),
            edtThisNode: $.proxy(this.edtThisNode, this)
    ...
    Tree.prototype._init = function (options) {
        ...
        //设置按钮显示
        buttons = this._options.button;
        //添加按钮函数名
        addNodeName = this._options.addNodeName;
        //删除按钮函数名
        delNodeName = this._options.delNodeName;
        //编辑按钮函数名
        edtNodeName = this._options.edtNodeName;
    ...
    */
    ...
    Tree.prototype._clickHandler = function (event) {
        ...
        //点击加号按钮
        if (classList.indexOf('add') !== -1) {
            //调用该名称的函数
            addNodeName = eval(addNodeName);
            addNodeName.call(this,event,node);

            this.expandNode(node, null);
        }
        //点击减号按钮
        else if (classList.indexOf('del') !== -1) {
            delNodeName = eval(delNodeName);
            delNodeName.call(this,event,node);
            this.delThisNode(node, null);
        }
        //点击笔按钮
        else if (classList.indexOf('edt') !== -1) {
            edtNodeName = eval(edtNodeName);
            edtNodeName.call(this,event,node);
        } else {
    ...
    Tree.prototype._setChecked = function (node, state, options) {
        if (node.length > 0) {
            for (var length = node.length, i = 0; i < length; i++) {
                // We never pass options when rendering, so the only time
                // we need to validate state is from user interaction
                if (options && state === node[i].state.checked)
                    return;

                // Highlight the node if its checkbox has unsaved changes
                if (this._options.highlightChanges) {
                    node[i].$el.toggleClass('node-check-changed', (this._checkedNodes.indexOf(node[i]) == -1) == state);
                }
                ...
                if (state) {
                    ...
                    // Set element
                    if (node[i].$el) {
                        node[i].$el.addClass('node-checked').removeClass('node-checked-partial');
                        node[i].$el.children('span.check-icon')
                    ...
                    // Optionally trigger event
                    this._triggerEvent('nodeChecked', node[i], options);
                }
                else if (state === undefined && this._options.hierarchicalCheck) {

                    // Set node state to partially checked
                    node[i].state.checked = undefined;

                    // Set element
                    if (node[i].$el) {
                        node[i].$el.addClass('node-checked-partial').removeClass('node-checked');
                        node[i].$el.children('span.check-icon')
                            .removeClass(this._options.uncheckedIcon)
                    ...
                    this._triggerEvent('nodeUnchecked', node[i], options);
                } else {
    ...
    Tree.prototype._renderNode = function (node, previousNode) {
        ...
        //add button if set button true run this code
        if (buttons) {
            node.$el.append(this._template.button.clone());
        }
    ...
    Tree.prototype._template = {
        ...
        button: $('<span id="addNode" class="glyphicon glyphicon-plus-sign add"></span> <span id="delNode" class="glyphicon glyphicon-minus-sign del"></span> <span id="edtNode" class="glyphicon glyphicon-pencil edt"></span>')
    };

    Tree.prototype._css = '.treeview .list-group-item{cursor:pointer}' +
        ...
        '.treeview span.add{left:2%;color: #d04de6;}' +
        '.treeview span.del{left:3%;color: #51bf6a;}' +
        '.treeview span.edt{left:4%;color: #44c3ad;}'
    ...
    Tree.prototype.addNodeAfter = function (nodes, node, options) {
        ...
        return this.addNode(nodes, this.getParents(node)[0], (node.index + 1), options);
    }
    ...
    Tree.prototype.updateNode = function (node, newNode, options) {
        ...
        targetNodes.splice(node.index, 1, newNode);//此处已经删除节点，并添加节点
    ...
    //tianyuan  添加额外的功能
    Tree.prototype.addChrildrenNode = function (nodes, options) {
        return this.addNodeAfter([{text: "未命名"}], {parentId: nodes.nodeId}, options);
    };
    Tree.prototype.delThisNode = function (nodes, options) {
        this.removeNode(nodes, options);
    };
    Tree.prototype.edtThisNode = function (node, newNode, options) {
        this.updateNode(node, newNode, options);
    };
